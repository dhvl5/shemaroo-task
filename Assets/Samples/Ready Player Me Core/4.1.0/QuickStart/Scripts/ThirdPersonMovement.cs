...
namespace ReadyPlayerMe.Samples
{
    ...
    public class ThirdPersonMovement : MonoBehaviour
    {
        ...
        public void Move(float inputX, float inputY)
        {
            ...
            JumpAndGravity();
            //_networkCharacterControllerPrototype.Move(moveDirection.normalized);
            _networkCharacterControllerPrototype.Move(moveDirection.normalized * (moveSpeed * Time.deltaTime) +  new Vector3(0.0f, verticalVelocity * Time.deltaTime, 0.0f));
        ...
        {
            if (_networkCharacterControllerPrototype.IsGrounded && verticalVelocity< 0)
            {
                verticalVelocity = -2f;
            }
            
            if (jumpTrigger && _networkCharacterControllerPrototype.IsGrounded)
            {
        ...
        {
            jumpTrigger = false;
            if (_networkCharacterControllerPrototype.IsGrounded)
            {
                jumpTrigger = true;
            }
            return jumpTrigger;
